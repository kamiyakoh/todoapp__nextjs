...
interface UseDemoData {
  hasComfirmed: boolean;
  handleHasComfirmed: () => void;
  fetch: () => void;
}
interface Todos {
  userId: number;
...
export const useDemoData = (): UseDemoData => {
  ...
  const [hasComfirmed, setHasComfirmed] = useState<boolean>(false);

  const handleHasComfirmed = (): void => {
    setHasComfirmed(true);
  };
  ...
  return { hasComfirmed, handleHasComfirmed, fetch };
};
